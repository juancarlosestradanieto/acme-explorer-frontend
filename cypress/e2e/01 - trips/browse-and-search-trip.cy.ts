...
describe('browse and search trip', () => {
  ...
  it('passes', () => {
    ...
    cy.fixture("login-credentials").then((data) => {
      ...
      //go to trip display page
      tripsPage.goToDisplayTrip(title);

      //ensure the trip is being shown
      cy.get(tripsPage.trip_card_title_selector)
      .contains(title)
      .should('be.visible');
